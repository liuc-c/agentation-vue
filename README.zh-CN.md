...
Toolbar 里的 `Agent 与 MCP` 面板会直接展示当前运行时配置，包括：
...
| `agent` | `{ enabled?: boolean, autoSend?: boolean } \| false` | `serve` 下默认启用，`autoSend` 默认关闭 | 启用本地 ACP companion agent bridge，并控制是否自动发送给当前 agent。 |
