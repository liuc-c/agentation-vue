...
agentation({
  sync: {
    ...
    projectId: "demo-app",                // 可选，默认取 workspace-name/relative-path；非 monorepo 时取 Vite 项目根目录名
    autoSync: true,
    debounceMs: 400,
    ensureServer: true,
  },
...
本地 agent bridge 默认关闭。如果你希望工具栏显示本地 agent 相关能力，需要显式开启：
...
共享服务模式下，多个本地项目可以共用同一个 companion 地址。Vite 插件会先做
health check，地址已被占用时直接复用现有进程，不会重复拉起。
...
默认情况下，统一 companion 服务监听 `4748`，浏览器同步走 `/v2/*`，
MCP 客户端在同一个端口上使用 streamable HTTP（`/mcp`）、兼容 SSE（`/sse`）或 stdio。

### 产品内快速开始

Toolbar 里的 `Companion 与 MCP` 面板会直接展示当前运行时配置；如果显式开启 agent，也会在同一处展示本地 agent 相关能力，包括：

- 浏览器同步 API 与 MCP transport 地址
- 当前项目可用的本地 agent 列表
- 自动发送 / 手动发送控制
- `agentation-vue-mcp server` 与 `agentation-vue-mcp agents init` 的 CLI 启动命令

推荐本地接法：

1. 多个 Vite 项目优先共用一套 Agentation 服务。
2. 在 monorepo 里，Vite 插件默认会把 `workspace-name/relative-path` 作为 `projectId`；非 monorepo 时回退到项目目录名。只有需要不同作用域时，再显式覆盖。
3. 多项目共用时，agent 读取批注请带 `projectFilter`。
4. 先让页面通过 ACP companion 直连本机 agent；只有还要接外部自动化时，再启用 webhook 转发。
...
| `sync` | `{ endpoint?: string, mcpEndpoint?: string, projectId?: string, autoSync?: boolean, debounceMs?: number, ensureServer?: boolean } \| false` | `serve` 下默认开启 | 把批注同步到统一的 Agentation companion 服务。 |
| `agent` | `{ enabled?: boolean, autoSend?: boolean } \| false` | 默认关闭，`autoSend` 默认关闭 | 启用本地 ACP companion agent bridge，并控制是否自动发送给当前 agent。 |
