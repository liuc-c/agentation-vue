...
describe("agentation plugin shared server startup", () => {
  ...
  it("still injects the inferred projectId when sync is disabled", async () => {
    const { agentation } = await import("./plugin.ts")
  ...
  it("keeps shared-server startup disabled for monorepo apps when sync is false", async () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "agentation-vue-"))
    const workspaceRoot = join(tempRoot, "agentation-vue")
    const projectRoot = join(workspaceRoot, "playgrounds", "vue-vite-demo")
    ...
    try {
      ...
      const end = vi.fn()
      middleware?.({ url: "/@agentation-vue/init.js" }, {
        statusCode: 0,
        setHeader: vi.fn(),
        end,
      }, vi.fn())
